...
- **Postponed (decision 2026-08-28):** actual migration of CI to the offline token is deferred until after the admin UI dashboard ships (~months). Interim consequence: the static `api_key` keeps its current (full) scope in prod longer than planned — acceptable, but issue #02 (JWT middleware) remains the priority so the UI/SSO path unblocks first.
